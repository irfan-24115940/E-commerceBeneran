...
function bindProductEvents(container, productList) {
  ...
  const productImgs = container.querySelectorAll('.product-img-clickable');
  ...
  productImgs.forEach(img => on(img, 'click', async (e) => {
    const id = parseInt(e.currentTarget.dataset.id);
    const product = await resolveProduct(id);
    if (product) {
...
function setupModals() {
  ...
  on('.close-team-modal', 'click', () => {
    if(teamModal) teamModal.style.display = 'none';
  });
  
  // Need to bind directly to document to catch multiple close buttons
  document.querySelectorAll('.close-product-modal').forEach(btn => {
    btn.onclick = () => { if(productModal) productModal.style.display = 'none'; };
  });
...
function showProductDetail(product) {
  ...
  const desc = product.description || 'Tidak ada deskripsi untuk produk ini.';
  const name = product.title || product.name;

  content.innerHTML = `
    <div style="display: flex; flex-wrap: wrap; gap: 2rem; align-items: flex-start;">
      <div style="flex: 1 1 300px;">
        <img src="${product.image}" alt="${name}" style="width: 100%; border-radius: 8px; object-fit: cover; aspect-ratio: 1/1;">
      </div>
      <div style="flex: 1 1 300px;">
        <span style="display: inline-block; padding: 4px 12px; background: #f3f4f6; border-radius: 9999px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 1rem; color: #4b5563;">${product.category || 'Uncategorized'}</span>
        <h2 style="font-size: 2rem; font-weight: 800; margin-bottom: 0.5rem; line-height: 1.2;">${name}</h2>
        <h3 style="font-size: 1.5rem; font-weight: 700; margin-bottom: 1.5rem; color: #111;">${formatRupiah(product.price)}</h3>
        
        <div style="margin-bottom: 2rem;">
          <h4 style="font-size: 0.875rem; text-transform: uppercase; font-weight: 700; color: #6b7280; margin-bottom: 0.5rem; letter-spacing: 0.05em;">Deskripsi Produk</h4>
          <p style="font-size: 0.95rem; line-height: 1.6; color: #374151; white-space: pre-wrap;">${desc}</p>
        </div>

        <div style="display: flex; gap: 1rem;">
          <button class="btn btn-buy-now-detail" data-id="${product.id}" style="flex: 1; padding: 1rem;">BELI SEKARANG</button>
          <button class="btn btn-add-cart-detail" data-id="${product.id}" style="flex: 1; padding: 1rem; background: transparent; border: 2px solid #111; color: #111;">+ KERANJANG</button>
        </div>
      </div>
    </div>
  `;
  ...
  const buyBtn = content.querySelector('.btn-buy-now-detail');
  const cartBtn = content.querySelector('.btn-add-cart-detail');

  buyBtn.onclick = async () => {
    if (!isLoggedIn()) { showToast('Silakan login terlebih dahulu!', 'error'); window.location.hash = '#login'; modal.style.display = 'none'; return; }
    await addToCart(product);
  ...
  cartBtn.onclick = async () => {
    if (!isLoggedIn()) { showToast('Silakan login terlebih dahulu!', 'error'); window.location.hash = '#login'; modal.style.display = 'none'; return; }
    const ok = await addToCart(product);
    if (ok) showToast(`${name} ditambahkan ke keranjang`);
    else showToast('Gagal menambahkan ke keranjang', 'error');
    modal.style.display = 'none';
  };
